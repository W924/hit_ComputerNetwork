...
client_sender = sr.SRSender(client_senderSocket, '127.0.0.1', 8888)

# client接收方
client_receiverSocket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
client_receiverSocket.bind(('127.0.0.1', 8080))
client_receiver = sr.SRReceiver(client_receiverSocket)
...
server_sender = sr.SRSender(server_senderSocket, '127.0.0.1', 8080)

# server接收方
server_receiverSocket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
server_receiverSocket.bind(('127.0.0.1', 8888))
server_receiver = sr.SRReceiver(server_receiverSocket)
